...
class Example extends React.Component {
	...
	render() {
		return (
			<div className='container Example'>
               ...
               <p>
                  Hello, My name is {this.state.name}. {this.state.motto}
               </p>
				</div>
				...
				<p>
					...
					&nbsp; named <span className='cs142-code-name'>Example</span>. It
					is located in the file{' '}
					<code>components/example/Example.jsx</code>. It looks like a
					JavaScript class named Example that has a method named named{' '}
					<span className='cs142-code-name'>render</span>, which appears to
					written in something that looks like HTML.
				</p>
				<p>
					...
					&nbsp; function which allow us to write something that looks like
					HTML to describe what the component renders.
				</p>
				<p>
					Although JSX looks like HTML, it is not HTML. Some of the
					differences are necessary due to embeddding in JavaScript. For
					example, rather than <code>class=</code> we use{' '}
					<code>className=</code> (class is a JavsScript keyword). Although
					it is possible to interleave JavaScript and JSX code, care is
					needed since contents of JSX tags are processed into arguments to
					a function limiting what can be done as will be seen below.
				</p>
				...
				<p>
					...
					expression can evaluate to a JavaScript string or value from a
					JSX expression. This feature allows componment&apos;s
					specification to use templated HTML.
				</p>

				<p>
					The Example class constructor sets the object&apos;s property{' '}
					<code>state.name</code> (see the assignment to{' '}
					<code>this.state.name</code> in <code>Example.jsx</code>) from
					the model in the DOM which has a value of &ldquo;{' '}
					{this.state.name}
					&rdquo; so:
				</p>
				...
				<p>
					React automatically propagates any changes to JavaScript state to
					the JSX templates. For example the following code{' '}
					<code>({'{this.state.counter}'})</code> displays the
					state.counter property of the Example component. The component
					sets a timer that increments the counter every 2 seconds. The
					value of the counter can be seen changing here:{' '}
					{this.state.counter}.
				</p>
				...
				<p>
					...
					JavaScript so we can use JavaScript language constructs for
					managing control flow.
				</p>
				<p>
					One way of doing control using JavaScript is to assign JSX
					fragements to JavaScript variables and use normal JavaScript
					control flow operators. For example, the following function
					selects among the posssible output lines based on an argument to
					the function and uses a for loop to populate an array. These
					JavaScript variables can then be referred to in JSX returned by
					the function.
				</p>
				<pre className='cs142-example-code'>
					<code className='language-jsx'>
  ...
  return retVal;
				...
				<p>
					Another way of accomplishing this is embedding the operations
					inside of curly braces. Although arbitrary JavaScript can appear
					inside braces, it must return a string or JSX expression to work.
					JavaScript control flow operations such as if, for, and while do
					not return values so templates like{' '}
					<code>{'{if (bool) ... else ...}'}</code> do not work.
				</p>
				<p>
  ...
  <ul>
    {[0,1,2].map((i) =>  <li key={i}>List Item {i}</li>)}
				...
				<p>
					Short-circuit boolean operations such as {'"&&"'} can also be
					used to control what is rendered. For example the following code
					will make a sentence appear between to two paragraph when some
...
`}
				...
				<div className='cs142-example-output'>
					...
					<p>
						... and this one when some characters are typed into the below
						box.
					</p>
				</div>
				...
				<p>
					Input in React is done using DOM-like event handlers. For
					example, JSX statements like:
				</p>
				<pre className='cs142-example-code'>
					<code className='language-jsx'>
...
<input type="text" value={this.state.inputValue} onChange={this.handleChangeBound} />`}
				...
				<p>
					...
					calls the function this.handleChangedBound every time the input
					field is changed. Typically this kind of input will be associated
					with a{' '}
					<a href='https://reactjs.org/docs/handling-events.html'>
						Button
					</a>{' '}
					or inside a{' '}
					<a href='https://reactjs.org/docs/forms.html'>Form</a> to allow
					the user to signal when they are finished changing the input
					field. Note the differences from HTML in <code>onchange=</code>{' '}
					become <code>onChange=</code> and
				...
				<p>
					The handleChangeBound function updates this.state.inputValue with
					the value of the DOM element so its value can be access like{' '}
					<code>{'{this.state.inputValue}'}</code> which returns
					&nbsp;&ldquo; {this.state.inputValue} &rdquo;. Note we can not
					directly call <code>this.handleChange</code> since it is a method
					function of an object that is not available to the DOM. To handle
					this we create a new function <code>this.handleChangeBound</code>{' '}
					that can call the method and use it here.
				</p>
				<p>
					If we want to pass arguments to event handling functions we can
					use inlined arrow function definitions like so:
				</p>
				<pre className='cs142-example-code'>
					<code className='language-jsx'>
						{`<div className="cs142-example-output">
  ...
  >
    Call handleButtonClick function with two
				...
				<p>
					When the button is pushed it will call the arrow function, which
					will then call the method <code>handleButtonClick</code> with the
					specified argument.
				</p>
				<div className='cs142-example-output'>
					<p>
