...
class UserPhotos extends React.Component {
   ...
   handleDeleteComment(event, photo_id, comment_id) {
		event.preventDefault();

      axios
         .delete(`/comment/${photo_id}`, {data: {comment_id: comment_id}})
         .then(() => {
            this.refresh();
         })
         .catch((e) => {
	...
	render() {
		return (
            ...
            {!this.state.userPhotos.length ? (
               <p>User has not uploaded any a photo</p>
            ): !this.props.checked ? (
                              ...
                              <Card key={comment._id} className='comment-section'>
                                 ...
                                 <CardContent className='comment'>
                                    ...
                                    {comment.user_id === this.props.userId && (
                                          <IconButton
                                             className='icon-btn'
                                             onClick={(event) => this.handleDeleteComment(event, photo._id, comment._id)}
                                          >
